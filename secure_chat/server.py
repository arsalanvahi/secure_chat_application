...
#  holds the random byte sequence generated by the server during secure login
@dataclass
class AuthenticationChallenge: #protocol data container for challenge-response communication style
    challenge_bytes:bytes

# represents the final outcome of the secure login process
@dataclass
...
# secure communication within a channel.
@dataclass
...
#a transport-level message used during
...
#  the transport-level response sent by the
# server after processing a client's enrollment request
@dataclass
...
# the first protocol message in the secure
# login process.
@dataclass
...
# a protocol packet sent from the
# server to the client during secure login.
@dataclass
...
# a protocol packet sent from the client
# to the server during challenge-response authentication.
@dataclass
...
# a final server-to-client message in the secure login protocol.
@dataclass
...
#  represents a protected channel message used for representing in GUI
@dataclass
...
#a protocol structure used when a client or server
# wants to terminate a session intentionally
@dataclass
class DisconnectMessage: #data container
    message_type:MessageType
    reason:str # Human-readable explanation for the disconnection

# represents an unexpected interruption in the
...
# defines the permanent user data stored by the server
...
# from the enrolled user's reversed password hash.
@dataclass
...
# during channel key generation on the server.
@dataclass
...
# =========================================
# overall runtime condition of the server
# keeps the server's global runtime snapshots and operational
# flags such as lifecycle state, channel availability, monitoring
# information
# What is server's overall state right now
# it uses ServerLifecylceState
# =========================================
class ServerRuntimeContext:
    def __init__(self):
        ...
        self.active_runtime_structure_registry = RuntimeStructureRegistry()  #reference to runtime objects that exists while only the server is active
        self.channel_availability_snapshot = { #whether each channel is available for secure communication
            ChannelName.IF100:False, #is IF100 ready?
            ChannelName.MATH101:False,
            ChannelName.SPS101:False
        }
        self.monitoring_snapshot = MonitoringSnapshot() #a runtime summary for monitoring
        self.retry_recovery_flags = RetryRecoveryState() #recovery related
    # for further features
    def get_lifecycle_state(self):
        pass
    #updates the stored lifecycle snapshot (for monitoring)
    ...
    def retrieve_runtime_structure(self):
        pass
    #after the ChannelKeyManager creates the runtime, the runtime context must reflect the channel is ready
    ...
    #returns whether the given channel is currently available
    def get_channel_availability(self,channel_name):
        if channel_name is None:
            return  False
        return self.channel_availability_snapshot.get(channel_name,False)
    #resets the entire runtime context back to its default sate
    #after this, server doesn't keep the global runtimes that are not fresh.
...
# =========================================
# Global function that reads tow PEM files from the disk and load the RSA private keys and
# sets in RsaKeySet
# =========================================
