...
def run_socket_secure_message_flow(sender_client, recipient_client, plaintext):
    print_step("SOCKET SECURE MESSAGE FLOW AFTER RECONNECT")
...
# =========================================================
def run_client_disconnect_and_fresh_reauth_flow(client_app, server_ip, server_port, username, password):
    print_step(f"CLIENT DISCONNECT AND FRESH RE-AUTH FLOW FOR {username}")

    # First authenticated session
    run_socket_authentication_flow(client_app, server_ip, server_port, username, password)

    # Client-initiated disconnect (button/window-close equivalent)
    disconnect_result = client_app.request_disconnect()
    print_kv("Client initiated disconnect result", disconnect_result)
    assert disconnect_result is True

    print_session_state("Client session state after client disconnect", client_app)
    assert client_app.client_session_manager.get_connection_state() is False
    assert client_app.client_session_manager.get_authentication_state() is False

    # Fresh connection + fresh authentication only (no enrollment)
    run_socket_authentication_flow(client_app, server_ip, server_port, username, password)


def observe_server_shutdown_from_client(client_app, label, timeout_seconds=4.0):
    """
    Wait until the client transport notices the server has closed the connection.
    This tests that the client understands the server-side shutdown.

    If the current client implementation does not automatically reset the logical
    session manager when transport loss occurs, the helper synchronizes the state
    so the test can continue without crashing.
    """
    print_step(f"OBSERVE SERVER SHUTDOWN FROM CLIENT {label}")
    ...
    while time.time() < deadline:
        try:
            last_message = client_app.client_connection_manager.receive_application_message()
        except Exception as error:
            print_kv(f"{label} receive exception during shutdown observation", error)
            last_message = None

        if last_message is None and client_app.client_connection_manager.detect_connection_loss():
            shutdown_observed = True
            break

        time.sleep(0.1)

    print_kv(f"{label} shutdown observed", shutdown_observed)
    print_kv(f"{label} transport connection_state", client_app.client_connection_manager.report_connection_state())
    print_kv(f"{label} last received message during shutdown observation", last_message)

    assert shutdown_observed is True, f"{label} did not detect server shutdown"

    # Synchronize logical client session state if current implementation
    # does not automatically reset on server-side disconnect.
    if client_app.client_session_manager.get_connection_state():
        print(f"{label}: transport detected server shutdown; synchronizing logical session state")
        client_app.client_session_manager.reset_session_state()

    print_session_state(f"{label} client session state after server shutdown", client_app)
    assert client_app.client_session_manager.get_connection_state() is False
    assert client_app.client_session_manager.get_authentication_state() is False
...
if __name__ == "__main__":
    print_banner("DISCONNECT / RECONNECT REQUIREMENTS TEST")
    ...
    # =================================================
    # PHASE 1: Start server, enroll users, test client disconnect/re-auth
    # =================================================
    print_banner("PHASE 1 - START SERVER AND ENROLL USERS")
    ...
    load_crypto_material(server1, [alice_client, bob_client], rsa_key_set)

    generate_channel_keys(
        server1,
        ChannelName.IF100,
        "if100_master_secret_disconnect_phase1"
    )
    ...
    try:
        # -------------------------------------------------
        # Enroll alice and bob once
        # -------------------------------------------------
        run_socket_registration_flow(
            alice_client,
            SERVER_IP,
            SERVER_PORT,
            username="alice_disconnect_test",
        ...
        run_socket_registration_flow(
            ...
            SERVER_IP,
            SERVER_PORT,
            username="bob_disconnect_test",
            password="bob_password_123",
            channel=ChannelName.IF100
        )

        # Clean disconnect after enrollment sockets
        run_socket_disconnect_flow(alice_client, server1.server_session_manager, username=None)
        run_socket_disconnect_flow(bob_client, server1.server_session_manager, username=None)

        # -------------------------------------------------
        # Client disconnect + fresh authentication without re-enrollment
        # -------------------------------------------------
        run_client_disconnect_and_fresh_reauth_flow(
            alice_client,
            SERVER_IP,
            SERVER_PORT,
            username="alice_disconnect_test",
        ...
        # Bob authenticates and stays connected too
        ...
        # Optional secure message before server shutdown
        run_socket_secure_message_flow(
            alice_client,
            bob_client,
            plaintext="Message before server-side shutdown"
        )

        # -------------------------------------------------
        # Server disconnects while clients are connected
        # -------------------------------------------------
        print_step("SERVER-SIDE SHUTDOWN WHILE CLIENTS ARE CONNECTED")
        server1.stop()

        # Clients must detect shutdown and not crash
        observe_server_shutdown_from_client(alice_client, "ALICE")
        observe_server_shutdown_from_client(bob_client, "BOB")

    finally:
        # Defensive cleanup
        try:
            alice_client.client_connection_manager.disconnect_from_server()
        except Exception:
            pass

        try:
            bob_client.client_connection_manager.disconnect_from_server()
        except Exception:
            pass

    # Give OS/socket stack time to settle
    time.sleep(1.5)

    # Inspect DB to confirm enrollments persisted
    inspect_enrollments_table(DB_FILE)

    # =================================================
    # PHASE 2: Restart server, reconnect using existing enrollments only
    # =================================================
    print_banner("PHASE 2 - RESTART SERVER AND RECONNECT EXISTING USERS")

    server2 = SocketServerHarness(SERVER_PORT)

    # Fresh clients after server restart
    alice_client_restarted = ClientAppCoordinator()
    bob_client_restarted = ClientAppCoordinator()

    load_crypto_material(server2, [alice_client_restarted, bob_client_restarted], rsa_key_set)

    # Channel keys are runtime-only, so regenerate after restart
    generate_channel_keys(
        server2,
        ChannelName.IF100,
        "if100_master_secret_disconnect_phase2"
    )
    ...
    try:
        # Fresh connections using previously completed enrollments only
        run_socket_authentication_flow(
            alice_client_restarted,
            SERVER_IP,
            SERVER_PORT,
            username="alice_disconnect_test",
            password="alice_password_123"
        ...
        # Optional message after restart to prove recovery path works
        run_socket_secure_message_flow(
            alice_client_restarted,
            bob_client_restarted,
            plaintext="Message after server restart using persisted enrollments"
        )

        # Duplicate registration should still fail after restart
        duplicate_client = ClientAppCoordinator()
        load_crypto_material(server2, [duplicate_client], rsa_key_set)
        ...
        print_banner("DISCONNECT / RECONNECT REQUIREMENTS TEST COMPLETED SUCCESSFULLY")

    finally:
        for client_app in [alice_client_restarted, bob_client_restarted]:
            try:
