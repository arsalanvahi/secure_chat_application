...
def connect_client_socket(client_app, server_ip, server_port):
    ...
    # Prevent hanging forever on recv()
...
if __name__ == "__main__":
    print_banner("FULL PERSISTENCE TEST START")
    ...
    # -------------------------------------------------
    # 1. Shared crypto material for both server runs
    # -------------------------------------------------
    rsa_key_set = build_rsa_key_set()

    # =================================================
    # PHASE 1: First server run -> register user
    # =================================================
    ...
    generate_channel_keys(
        server1,
        ChannelName.IF100,
        "if100_master_secret_persistence_phase1"
    )
    ...
    try:
        ...
        # Disconnect registration client cleanly before shutdown
    ...
    # Give the old server more time to fully stop before restart
    time.sleep(1.5)

    # Confirm DB file now exists
    ...
    # Inspect DB contents after Phase 1
    inspect_enrollments_table(DB_FILE)

    # =================================================
    # PHASE 2: Second server run -> authenticate without re-registering
    # =================================================
    print_banner("PHASE 2 - SECOND SERVER RUN")

    server2 = SocketServerHarness(SERVER_PORT)

    auth_client = ClientAppCoordinator()
    duplicate_client = ClientAppCoordinator()

    load_crypto_material(server2, [auth_client, duplicate_client], rsa_key_set)

    # IMPORTANT: channel keys are runtime-only, so regenerate after restart
    generate_channel_keys(
        server2,
        ChannelName.IF100,
        "if100_master_secret_persistence_phase2"
    )
    ...
    try:
        # ---------------------------------------------
        # 2A. Authenticate previously registered user
        # ---------------------------------------------
        ...
        # ---------------------------------------------
        # 2B. Duplicate registration should still fail
        # ---------------------------------------------
        ...
        # ---------------------------------------------
        # 2C. Graceful disconnect after persistence auth
        # ---------------------------------------------
        run_socket_disconnect_flow(
            auth_client,
        ...
        print_banner("FULL PERSISTENCE TEST COMPLETED SUCCESSFULLY")

    finally:
        try:
            auth_client.client_connection_manager.disconnect_from_server()
        except Exception:
            pass
