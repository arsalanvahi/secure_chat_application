...
# =========================================================
# Socket server runner (without changing your module skeleton)
# =========================================================
class SocketServerHarness:
    ...
    def _accept_loop(self):
        while not self.stop_event.is_set():
            ...
            # IMPORTANT:
            # Pre-register a server-side session entry so authentication activation works.
    ...
    def _connection_loop(self, connection_id):
        while not self.stop_event.is_set():
            ...
            # Only send protocol messages back directly.
            # RelayResult for MSG_SEND is not a protocol packet and should not be serialized back.
...
def connect_client_socket(client_app, server_ip, server_port):
    ...
    # Sync session manager with connection manager
...
def run_socket_secure_message_flow(sender_client, recipient_client, plaintext):
    ...
    # Recipient receives relayed secure packet
    ...
    # Optional: consume sender echo if your server broadcasts back to sender too
    try:
...
if __name__ == "__main__":
    print_banner("FULL SOCKET-MODE FLOW TEST START")
    ...
    # 3. RSA setup
    # -------------------------------------------------
    rsa_key_set = initialize_socket_mode_crypto(
    ...
    try:
        ...
        print_banner("FULL SOCKET-MODE FLOW COMPLETED SUCCESSFULLY")
